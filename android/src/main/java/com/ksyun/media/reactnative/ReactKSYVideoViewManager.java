...
import com.github.sondt87.reactnative.videoplayer.ExoPlayerView;
...
public class ReactKSYVideoViewManager extends SimpleViewManager<AbsVideoView> {
...
//        final AbsVideoView mVideoView = new ReactKSYVideoView(reactContext);
        final AbsVideoView mVideoView = new ExoPlayerView(reactContext);
        return mVideoView;
    }

    @Override
    public void onDropViewInstance(AbsVideoView view) {//销毁对象
    ...
    @Override
    public void receiveCommand(AbsVideoView video, int commandId, @Nullable ReadableArray args) {
        switch (commandId){
    ...
    public void setSource(AbsVideoView videoView, @Nullable ReadableMap src){
        String source = src.getString(PROP_SRC_URI);
        videoView.setDataSource(source);
    }

    @ReactProp(name = PROP_RESIZE_MODE)
    public void setResizeMode(final AbsVideoView videoView, final String resizeModeOrdinalString) {
    ...
    public void setPause (AbsVideoView videoView, @Nullable boolean paused){
        videoView.setPausedModifier(paused);
    }

    @ReactProp(name = PROP_MIRROR, defaultBoolean = false)
    public void setMirror(AbsVideoView videoView, @Nullable boolean mirror){
        videoView.setMirror(mirror);
    }

    @ReactProp(name = PROP_DEGREE, defaultInt = 0)
    public void setDegree(AbsVideoView videoView, @Nullable int degree){
        videoView.setRotateDegree(degree);
    }

    @ReactProp(name = PROP_MUTED, defaultBoolean = false)
    public void setMuted(final AbsVideoView videoView, final boolean muted) {
        videoView.setMutedModifier(muted);
    }

    @ReactProp(name = PROP_VOLUME, defaultFloat = 0.5f)
    public void setVolumn(AbsVideoView videoView, @Nullable double volume){
        videoView.setVolumeModifier((float) volume);
    }


    @ReactProp(name = PROP_REPEAT, defaultBoolean = false)
    public void setRepeat(final AbsVideoView videoView, final boolean repeat) {
        videoView.setRepeatModifier(repeat);
    }

    @ReactProp(name = PROP_PROGRESS_UPDATE_INTERVAL, defaultFloat = 250.0f)
    public void setProgressUpdateInterval(final AbsVideoView videoView, final float progressUpdateInterval) {
        videoView.setProgressUpdateInterval(progressUpdateInterval);
    }

    @ReactProp(name = PROP_SEEK)
    public void setSeek(final AbsVideoView videoView, final float seek) {
        videoView.seekToModifier(Math.round(seek * 1000.0f));
    }

    @ReactProp(name = PROP_PLAY_IN_BACKGROUND, defaultBoolean = false)
    public void setPlayInBackground(final AbsVideoView videoView, final boolean playInBackground) {
        videoView.setPlayInBackground(playInBackground);
    }

    @ReactProp(name = PROP_CONTROLS, defaultBoolean = false)
    public void setControls(final AbsVideoView videoView, final boolean controls) {
        videoView.setControls(controls);
    }

    @ReactProp(name = PROP_TIMEOUT)
    public void setTimeout(final AbsVideoView videoView, @Nullable ReadableMap timeout) {
    ...
    public void setBufferSize(final AbsVideoView videoView, final int bufferSize) {
        videoView.setBufferSize(bufferSize);
    }

    @ReactProp(name = PROP_BUFFERTIME,  defaultInt = 2)
    public void setBufferTime(final AbsVideoView videoView, final int bufferTime) {
